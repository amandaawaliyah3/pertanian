                ...
                ->schema([
                    tap(FileUpload::make('logo'), function (FileUpload $fileUpload) {
                        $fileUpload->image()
                            ...
                            // ✅ FIX UTAMA: Naming file yang disederhanakan untuk menghindari Type Error
                            ...
                            // 1. Izinkan null saat Edit (untuk replace/reset)
                            ->nullable(fn (string $operation): bool => $operation === 'edit')
                            
                            // 2. Hook untuk menghapus logo lama saat diganti atau di-reset
                            ->mutateDehydratedState(function ($state, $old, $livewire) {
                                if ($livewire instanceof \Filament\Resources\Pages\EditRecord) {
                                    $oldRecord = $livewire->getRecord();
                                    
                                    // Jika ada path lama DAN path baru berbeda (replace/reset)
        ...
        return $table
            ->columns([
                ImageColumn::make('logo')
                    ->disk('public') // ✅ Tampilkan logo di tabel
                    ->circular(),
